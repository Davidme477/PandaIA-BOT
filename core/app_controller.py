...
class PandaWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            self.status_changed.emit(
                "kokoro_missing",
                "Kokoro todavía no está instalado.",
            )
    ...
    def ensure_overlay(self) -> None:
        ...
        for _ in range(30):
            ...
            if (
                ...
                raise RuntimeError(
                    "El overlay se cerró durante el inicio."
                )

            time.sleep(0.2)

        raise RuntimeError(
            "El overlay no respondió."
        )
...
class AppController(QObject):
    ...
    def connect_all(
        ...
        self.tiktok_status_changed.emit(
            "CONECTANDO",
            "statusRed",
        )

        self.ollama_status_changed.emit(
            "VERIFICANDO",
            "statusRed",
        )
        ...
        self.log_message.emit(
            f"Iniciando PandaIA para {username}."
        )
        ...
        self.worker.activity_received.connect(
    ...
    def change_voice(self) -> None:
        self.log_message.emit(
            "Cambiar voz seleccionado. "
            "Kokoro TTS todavía no está instalado."
        )
